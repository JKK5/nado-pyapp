...
# 화면 타이틀 설정
pygame.display.set_caption('JKK5 Pang')
...
# 캐릭터 이동 방향
character_to_x_LEFT = 0  # 왼쪽
character_to_x_RIGHT = 0   # 오른쪽
...
# 게임 종료 메시지
# Mission Complete(성공), Time Over(시간 초과 실패), Game Over(공에 맞음)
...
while running:
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_LEFT:  # 캐릭터를 왼쪽으로
                ...
                weapon_x_pos = character_x_pos + \
                    (character_width - weapon_width) / 2
    ...
    for ball_idx, ball_val in enumerate(balls):
        ...
        break  # 안쪽 for문에서 break를 만나면 여기로 진입, 이중 for문을 한 번에 탈출
